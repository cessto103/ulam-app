...
export default function CreateRecipeScreen() {
  ...
  const photoURIs = photos.map((a) => a.uri);
  ...
  const pickPhoto = async () => {
    ...
    const result = await ImagePicker.launchImageLibraryAsync({
      ...
      selectionLimit: MAX_PHOTOS - photos.length, quality: 1,
    });
    if (!result.canceled) {
      const wasEmpty  = photos.length === 0;
      const newPhotos = [...photos, ...result.assets].slice(0, MAX_PHOTOS);
      setPhotos(newPhotos);
      // Auto-open style picker the first time photos are added
      if (wasEmpty && newPhotos.length > 0) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      await client.post('/recipes', form, { headers: { 'Content-Type': 'multipart/form-data' } });
      qc.invalidateQueries({ queryKey: ['recipes'] });
      Alert.alert(
        'Recipe saved!',
        'Your recipe is private. Go to the Recipes tab and tap the share toggle to publish it.',
        [{ text: 'OK', onPress: () => router.back() }],
      );
    } catch (e: unknown) {
      const err = e as { response?: { data?: { message?: string } } };
      Alert.alert('Error', err?.response?.data?.message ?? 'Could not save recipe. Please try again.');
    } finally {
      setLoading(false);
    }
  }, [title, description, youtubeUrl, servings, prep, cook, difficulty, budgetTag, collageStyle, gradientKey, fontKey, tags, steps, tips, ingredients, photos]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ── Cover preview — full bleed, no side padding ── */}
        <RecipeCoverPhoto
          photos={photoURIs}
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 16, paddingTop: 10 }}>
          ...
          <Pressable
            ...
            style={{
              ...
              backgroundColor: '#FDF0EA', borderRadius: 10,
            }}
          >
        ...
        <View style={{ paddingHorizontal: 16 }}>
          ...
          {/* Estimated cost */}
          <View style={{ backgroundColor: '#FDF0EA', borderRadius: 12, paddingHorizontal: 16, paddingVertical: 12, marginBottom: 24, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          >
            {loading ? (
              ...
              <Text style={{ fontFamily: 'NunitoSans_700Bold', fontSize: 14, color: '#fff' }}>Save Recipe</Text>
            )}
          </Pressable>
